...
export class AutoDefiner {
  ...
  const crawlTree = (rootNode) => {
    ...
    if (!rootNode) return;
    // Scan this node if it's an element
    ...
    // Scan descendants (works for Element + ShadowRoot)
  ...
  const observeShadowRoot = (sr) => {
    ...
    const mo = new MutationObserver((mutations) => {
      for (const m of mutations) {
        m.addedNodes?.forEach((n) => crawlTree(n));
        if (m.type === "attributes" && m.target) crawlTree(m.target);
      }
    });
    mo.observe(sr, {
      childList: true,
      subtree: true,
      attributes: true,
      attributeFilter: [attributeModule],
    });
